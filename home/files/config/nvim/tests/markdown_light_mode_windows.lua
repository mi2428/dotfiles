...
assert(not vim.wo[new_big_win].cursorline and not vim.wo[new_big_win].wrap, "split bigfile guards missing")
...
assert(
	vim.wait(1000, function()
		return mini_map.current.buf_data.source == small_buf
	end, 20),
	"bigfile replaced the normal peer as mini.map source"
)
